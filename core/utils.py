...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        c1, c2 = (coor[1], coor[0]), (coor[3], coor[2]) # c1: 시작점 좌표 (x1, y1)/ c2: 종료점 좌표 (x2, y2)
        center= (int((coor[1] + coor[3])/2), int((coor[0] + coor[2])/2)) 
        cv2.rectangle(image, c1, c2, bbox_color, bbox_thick)
        cv2.line(image, center, center, (0,0,255), 5)
...
def cut_box(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
    ...
    for i in range(num_boxes[0]):
        ...
        image_crop = image[(center[0]-10):(center[0] +10), (center[1] - 10):(center[1] +10)]
        print((center[0]-10), (center[0] +10))
        print((center[1] - 10), (center[1] +10))
